...
keycloak.init({ onLoad: initOptions.onLoad }).success((auth) => {
  ...
  console.log(keycloak["tokenParsed"]["realm_access"])
  console.log(keycloak.token)

  localStorage.setItem("react-token", keycloak.token);
  localStorage.setItem("react-refresh-token", keycloak.refreshToken);

    if(keycloak["tokenParsed"]["realm_access"]["roles"].includes("app-dac-admin")){
        localStorage.setItem("role", 'dac-admin');
    } else if (keycloak["tokenParsed"]["realm_access"]["roles"].includes("app-dac-member")){
