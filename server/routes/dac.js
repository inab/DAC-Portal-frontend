...
export default ({ keycloak }) => {
	...
	api.get('/data', keycloak.protect('is-dac'), async function(req, res){
		const userInfo = jwt_decode(req.headers.authorization)
                const response = await getDacData(userInfo.sub);
		res.send(response)
        })
        api.get('/requests', keycloak.protect('is-dac'), async function(req,res) {
                const userInfo = jwt_decode(req.headers.authorization)
        ...
        api.put('/policies', keycloak.protect('dac-admin'), async function(req,res) {
                const userInfo = jwt_decode(req.headers.authorization)
                const dacId = req.param('dac-id');
                const fileId = req.param('ds-id');
                const policy = req.param('policy');
                const response = await updatePolicies(userInfo.sub, dacId, fileId, policy);
		res.send(response)
        })
